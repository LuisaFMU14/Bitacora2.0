...
def transcribe_audio():
    try:
        ...
        print("📥 Audio recibido:", file.filename)

        # Guardar audio WebM temporalmente
        temp_webm = tempfile.NamedTemporaryFile(delete=False, suffix=".webm")
        file.save(temp_webm.name)
        print("📦 Guardado como:", temp_webm.name)

        # Convertir WebM ➝ WAV usando pydub
        ...
        if result.reason == speechsdk.ResultReason.RecognizedSpeech:
            print("✅ Transcripción exitosa:", result.text)
            return jsonify({"text": result.text})
        else:
            print("⚠️ Azure no reconoció el audio:", result.reason)
